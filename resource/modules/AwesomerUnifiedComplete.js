...
/* This file is based on (almost a complete copy in fact) of
...
const { manager: Cm, classes: Cc, interfaces: Ci, results: Cr, utils: Cu } = Components;

Cu.import("resource://gre/modules/Services.jsm");
var gFx47 = (Services.vc.compare(Services.appinfo.version, "47.0a1") >= 0);
var gFx48 = (Services.vc.compare(Services.appinfo.version, "48.0a1") >= 0);
var gFx49 = (Services.vc.compare(Services.appinfo.version, "49.0a1") >= 0);
var gFx50 = (Services.vc.compare(Services.appinfo.version, "50.0a1") >= 0);
var gFx51 = (Services.vc.compare(Services.appinfo.version, "51.0a1") >= 0);
var gFx52 = (Services.vc.compare(Services.appinfo.version, "52.0a1") >= 0);
...
function defaultQuery(conditions = "") {
	let query =
		`SELECT :query_type, h.url, h.title, f.url, ${SQL_BOOKMARK_TAGS_FRAGMENT}, h.visit_count, h.typed, h.id, t.open_count, h.frecency
		 FROM moz_places h
...
const SQL_SWITCHTAB_QUERY =
	`SELECT :query_type, t.url, t.url, NULL, NULL, NULL, NULL, NULL, NULL, NULL, t.open_count, NULL
...
const SQL_ADAPTIVE_QUERY =
	`/* do not warn (bug 487789) */
	 SELECT :query_type, h.url, h.title, f.url, ${SQL_BOOKMARK_TAGS_FRAGMENT}, h.visit_count, h.typed, h.id, t.open_count, h.frecency
	 FROM (
		 SELECT ROUND(MAX(use_count) * (1 + (input = :search_string)), 1) AS rank, place_id
...
XPCOMUtils.defineLazyModuleGetter(this, "Sqlite", "resource://gre/modules/Sqlite.jsm");
XPCOMUtils.defineLazyModuleGetter(this, "OS", "resource://gre/modules/osfile.jsm");
XPCOMUtils.defineLazyModuleGetter(this, "PromiseUtils", "resource://gre/modules/PromiseUtils.jsm");
XPCOMUtils.defineLazyModuleGetter(this, "ExtensionSearchHandler", "resource://gre/modules/ExtensionSearchHandler.jsm");
...
if(!gFx52) {
	...
	XPCOMUtils.defineLazyGetter(this, "SwitchToTabStorage", () => Object.seal({
		...
		delete: function(uri, userContextId) {
			if(!this._conn) {
				// This should not happen.
				if(!this._queue.has(userContextId)) { return; }
...
XPCOMUtils.defineLazyGetter(this, "Prefs", () => {
	...
	function loadPrefs(subject, topic, data) {
		...
		store.defaultBehavior = types.concat("Typed").reduce((memo, type) => {
			...
			return memo | (prefValue && Ci.mozIPlacesAutoComplete["BEHAVIOR_" + type.toUpperCase()]);
		}, 0);
		...
		if(store.suggestHistory) {
			store.emptySearchDefaultBehavior |= Ci.mozIPlacesAutoComplete.BEHAVIOR_HISTORY | Ci.mozIPlacesAutoComplete.BEHAVIOR_TYPED;
		...
		if(store.matchBehavior != MATCH_ANYWHERE && store.matchBehavior != MATCH_BOUNDARY && store.matchBehavior != MATCH_BEGINNING) {
...
function looksLikeUrl(str, ignoreAlphanumericHosts = false) {
	if(!gFx52) {
		...
		return !REGEXP_SPACES.test(str) && ["/", "@", ":", "."].some(c => str.includes(c));
	}

	// Single word not including special chars.
	return !REGEXP_SPACES.test(str) && (["/", "@", ":", "["].some(c => str.includes(c)) || (ignoreAlphanumericHosts ? /(.*\..*){3,}/.test(str) : str.includes(".")));
}
 ...
 */
function Search(searchString, searchParam, autocompleteListener, resultListener, autocompleteSearch, prohibitSearchSuggestions) {
	...
	this._behavior = this._searchString ? Prefs.defaultBehavior : Prefs.emptySearchDefaultBehavior;
	...
	if(gFx51) {
		...
		this._userContextId = userContextId ? parseInt(userContextId[1], 10) : Ci.nsIScriptSecurityManager.DEFAULT_USER_CONTEXT_ID;
	}

	this._searchTokens = this.filterTokens(getUnfilteredSearchTokens(this._searchString));
	...
	this._autofillUrlSearchString = fixupSearchText(
		this._trimmedOriginalSearchString.slice(0, pathIndex).toLowerCase() + this._trimmedOriginalSearchString.slice(pathIndex)
	);
...
Search.prototype = {
	...
	hasBehavior: function(type) {
		...
		if(this._disablePrivateActions && behavior == Ci.mozIPlacesAutoComplete.BEHAVIOR_OPENPAGE) {
			return false;
		}
	...
	_sleep: function(aTimeMs) {
		...
		this._sleepTimer.initWithCallback(() => this._sleepDeferred.resolve(), aTimeMs, Ci.nsITimer.TYPE_ONE_SHOT);
	...
	execute: Task.async(function* (conn) {
		...
		if(!gFx50 || hasHeuristic) {
			yield this._sleep(Prefs.delay);
			if(!this.pending) { return; }
		}

		if(!gFx49 || (this._enableActions && (!gFx51 || this._searchTokens.length > 0))) {
		...
		if(gFx52) {
			if(ExtensionSearchHandler.isKeywordRegistered(this._searchTokens[0]) && this._originalSearchString.length > this._searchTokens[0].length) {
	...
	_matchFirstHeuristicResult: function* (conn) {
		...
		if((gFx51 || this.pending) && this._enableActions && (!gFx51 || hasSearchTerms)) {
		...
		if(gFx51 && this.pending && hasSearchTerms) {
		...
		if(this.pending && (!gFx51 || hasSearchTerms) && this._enableActions) {
			...
			if(matched) {
				if(gFx52) {
					// Since we can't tell if this is a real URL and whether the user wants to visit or search for it,
	...
	_matchSearchSuggestions: function* () {
		...
		let promise = this._searchSuggestionController.fetchCompletePromise.then(() => {
			...
			if(!this._searchSuggestionController) { return; }

			if(this._searchSuggestionController.resultsCount >= 0 && this._searchSuggestionController.resultsCount < 2) {
	...
	_prohibitSearchSuggestionsFor: function(searchString) {
		...
		if(!gFx51) {
			...
			if(REGEXP_SINGLEWORD_HOST.test(tokens[0]) && Services.uriFixup.isDomainWhitelisted(tokens[0], -1)) {
				return true;
			}
	...
	_matchPlacesKeyword: function* () {
		...
		if(!gFx51) {
			...
			url = entry.url.href.replace("%s", queryString);
		}
		else {
			let searchString = this._trimmedOriginalSearchString.substr(keyword.length + 1);
			try {
				[url, postData] = yield BrowserUtils.parseUrlAndPostData(entry.url.href, entry.postData, searchString);
			}
			catch (ex) {
		...
		let actionURL;
		if(!gFx50) {
			actionURL = makeActionURL("keyword", { url, input: this._originalSearchString });
		} else if(!gFx51) {
			actionURL = PlacesUtils.mozActionURI("keyword", { url, input: this._originalSearchString });
		} else {
			actionURL = PlacesUtils.mozActionURI("keyword", { url, input: this._originalSearchString, postData });
		}
	...
	_matchSearchEngineUrl: function* () {
		...
		let match = yield PlacesSearchAutocompleteProvider.findMatchByToken(this._searchString);
		if(!match) {
			return false;
		}
		...
		if(this._strippedPrefix.endsWith("www.") && !stripHttpAndTrim(match.url).startsWith("www.")) {
			return false;
		}
		...
		if(!value.startsWith(this._originalSearchString)) {
			Components.utils.reportError(`Trying to inline complete in-the-middle ${this._originalSearchString} to ${value}`);
			return false;
		}
	...
	_matchSearchEngineAlias: function* () {
		...
		let query = (!gFx51) ? this._searchTokens.slice(1).join(" ") : this._trimmedOriginalSearchString.substr(alias.length + 1);
	...
	_addExtensionMatch(content, comment) {
		if(this._extensionMatchesCount >= MAXIMUM_ALLOWED_EXTENSION_MATCHES) { return; }
	...
	_addSearchEngineMatch(match, query, suggestion) {
		...
		let value = (!gFx50) ? makeActionURL("searchengine", actionURLParams) : PlacesUtils.mozActionURI("searchengine", actionURLParams);
	...
	_matchExtensionSuggestions: function* () {
		let promise = ExtensionSearchHandler.handleSearch(this._searchTokens[0], this._originalSearchString, (suggestions) => {
	...
	_matchRemoteTabs: function* () {
		...
		for(let { url, title, icon, deviceClass, deviceName } of matches) {
			// It's rare that Sync supplies the icon for the page (but if it does, it is a string URL)
			if(!icon) {
				try {
				...
				icon = PlacesUtils.favicons.getFaviconLinkForIcon(NetUtil.newURI(icon)).spec;
			}

			let match = {
				// We include the deviceName in the action URL so we can render it in the URLBar.
				value: (!gFx50) ? makeActionURL("remotetab", { url, deviceName }) : PlacesUtils.mozActionURI("remotetab", { url, deviceName }),
				comment: title || url,
				style: (!gFx48) ? "action" : "action remotetab",
				// we want frecency > FRECENCY_DEFAULT so it doesn't get pushed out by "remote" matches.
	...
	_matchUnknownUrl: function* () {
		let flags = Ci.nsIURIFixup.FIXUP_FLAG_FIX_SCHEME_TYPOS | Ci.nsIURIFixup.FIXUP_FLAG_ALLOW_KEYWORD_LOOKUP;
		let fixupInfo = null;
		try {
			fixupInfo = Services.uriFixup.getFixupURIInfo(this._originalSearchString, flags);
		} catch(e) {
			return false;
		}
		...
		let value;
		if(!gFx50) {
			value = makeActionURL("visiturl", {
				url: escapedURL,
				input: this._originalSearchString,
			});
		} else {
			value = PlacesUtils.mozActionURI("visiturl", {
				url: escapedURL,
				input: this._originalSearchString,
			});
		}
	...
	_maybeRestyleSearchMatch: function(match) {
		...
		if(this._searchTokens.length > 0 && this._searchTokens.every(token => !terms.includes(token))) { return; }

		if(!gFx48) {
			// Use the special separator that the binding will use to style the item.
			match.style = "search " + match.style;
			match.comment = parseResult.terms + TITLE_SEARCH_ENGINE_SEPARATOR + parseResult.engineName;
		}
		else {
	...
	_addMatch: function(match) {
		...
		if((match.placeId && this._usedPlaceIds.has(match.placeId)) || this._usedURLs.has(urlMapKey)) { return; }

		// Add this to our internal tracker to ensure duplicates do not end up in the result.
		// Not all entries have a place id, thus we fallback to the url for them.
		// We cannot use only the url since keywords entries are modified to include the search string, and would be returned multiple times.
		// Ids are faster too.
		if(match.placeId) {
		...
		match.icon = match.icon || ((!gFx48) ? PlacesUtils.favicons.defaultFavicon.spec : "");
		match.finalCompleteValue = match.finalCompleteValue || "";

		this._result.insertMatchAt(this._getInsertIndexForMatch(match), match.value, match.comment, match.icon, match.style, match.finalCompleteValue);
	...
	_processHostRow: function(row) {
		...
		// If the untrimmed value doesn't preserve the user's input just ignore it and complete to the found host.
		if(untrimmedHost && !untrimmedHost.toLowerCase().includes(this._trimmedOriginalSearchString.toLowerCase())) {
			untrimmedHost = null;
		}
		...
		if(faviconUrl) {
			match.icon = PlacesUtils.favicons.getFaviconLinkForIcon(NetUtil.newURI(faviconUrl)).spec;
		}
		// Although this has a frecency, this query is executed before any other queries that would result in frecency matches.
	...
	_processUrlRow: function(row) {
		...
		let separatorIndex = url.slice(this._searchString.length).search(/[\/\?\#]/);
		if(separatorIndex != -1) {
		...
		// If the untrimmed value doesn't preserve the user's input just ignore it and complete to the found url.
		let untrimmedURL = prefix + url;
		if(untrimmedURL && !untrimmedURL.toLowerCase().includes(this._trimmedOriginalSearchString.toLowerCase())) {
			untrimmedURL = null;
		 }
	...
	_processRow: function(row) {
		...
		let bookmarkTitle = bookmarked ? row.getResultByIndex(QUERYINDEX_BOOKMARKTITLE) : null;
		let tags = row.getResultByIndex(QUERYINDEX_TAGS) || "";
		let frecency = row.getResultByIndex(QUERYINDEX_FRECENCY);

		// If actions are enabled and the page is open, add only the switch-to-tab result. Otherwise, add the normal result.
		let url = escapedURL;
		let action = null;
		if(this._enableActions && openPageCount > 0 && this.hasBehavior("openpage")) {
			url = (!gFx50) ? makeActionURL("switchtab", { url: escapedURL }) : PlacesUtils.mozActionURI("switchtab", { url: escapedURL });
			action = "switchtab";
		}
		...
		if(this.hasBehavior("history") && !this.hasBehavior("bookmark") && !showTags) {
			showTags = false;
			match.style = "favicon";
		}
		...
		match.comment = title;
		if(iconurl) {
			match.icon = PlacesUtils.favicons.getFaviconLinkForIcon(NetUtil.newURI(iconurl)).spec;
		}
		match.frecency = frecency;
	...
	get _suggestionPrefQuery() {
		if(!this.hasBehavior("restrict") && this.hasBehavior("history") && this.hasBehavior("bookmark")) {
			return this.hasBehavior("typed") ? defaultQuery("AND h.typed = 1") : defaultQuery();
		}
		let conditions = [];
		...
		return conditions.length ? defaultQuery("AND " + conditions.join(" AND ")) : defaultQuery();
	},
	...
	get _searchQuery() {
		...
		if(gFx52) {
			params.userContextId = this._userContextId;
		}
	...
	get _switchToTabQuery() {
		...
		return [ SQL_SWITCHTAB_QUERY, params ];
	},
	...
	get _adaptiveQuery() {
		...
		return [ SQL_ADAPTIVE_QUERY, params ];
	},
	...
	get _hostQuery() {
		...
		let query = [];
		if(bookmarked) {
			query.push(typed ? SQL_BOOKMARKED_TYPED_HOST_QUERY : SQL_BOOKMARKED_HOST_QUERY);
		} else {
			query.push(typed ? SQL_TYPED_HOST_QUERY : SQL_HOST_QUERY);
		}
	...
	get _urlQuery() {
		...
		let revHost = this._autofillUrlSearchString.substring(0, slashIndex).toLowerCase().split("").reverse().join("") + ".";
		...
		let query = [];
		if(bookmarked) {
			query.push(typed ? SQL_BOOKMARKED_TYPED_URL_QUERY : SQL_BOOKMARKED_URL_QUERY);
		} else {
			query.push(typed ? SQL_TYPED_URL_QUERY : SQL_URL_QUERY);
		}
...
// component @mozilla.org/autocomplete/search;1?name=awesomerunifiedcomplete
...
UnifiedComplete.prototype = {
	...
	registerOpenPage: function(uri, userContextId) {
		if(!gFx52) {
	...
	unregisterOpenPage: function(uri, userContextId) {
		if(!gFx52) {
	...
	startSearch: function(searchString, searchParam, previousResult, listener) {
		...
		let prohibitSearchSuggestions = this._lastLowResultsSearchSuggestion
		...
		this._currentSearch = new Search(searchString, searchParam, listener, this, this, prohibitSearchSuggestions);
		...
		this.getDatabaseHandle().then(conn => search.execute(conn)).then(null, ex => {
	...
	classDescription: "An enhanced fork of Firefox's native UnifiedComplete",
	classID: Components.ID("879df050-c1be-11e5-a837-0800200c9a66"),
	contractID: '@mozilla.org/autocomplete/search;1?name=awesomerunifiedcomplete',
	...
	_load: function() {
		if(!gFactory) {
			gFactory = XPCOMUtils._getFactory(UnifiedComplete);
			Cm.QueryInterface(Ci.nsIComponentRegistrar).registerFactory(this.classID, this.classDescription, this.contractID, gFactory);
		}
	},
